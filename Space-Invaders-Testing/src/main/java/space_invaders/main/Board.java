...
public class Board extends JPanel {
    ...
    /**
     *Actualiza los el estado de los aliens, Mueve el alienígena hacia la izquierda o a la derecha una posición en función de la
     *dirección indicada (direction=1 izquierda, direction=-1 derecha). Si se ha alcanzado el borde del tablero y no se puede mover
     *el alienígena en la dirección indicada, se cambia de dirección y se mueven todos los alienígenas una posición hacia abajo (Commons.GO_DOWN).
  ...
  /*
  Actualiza las bombas de los alenígenas. Genera una bomba por alienígena. Si una bomba ha sido destruida, o no ha sido creada por primera vez,
   la crea asignándole las coordenadas actuales del alienígena, y establece su estado "destroyed" a falso. Si la bomba ha llegado al suelo
   (Commons.GROUND-Commons.BOMB_HEIGHT), se destruye y desaparece (cambia destroyed a true). Si el jugador ha sido alcanzado por una bomba,
   el jugador cambiará su estado "setDying" a verdadero, y su imagen se cambiará por la animación de explosión Si no sucede ninguna de las
        ...
        for (Alien alien : this.aliens) {
            ...
            if (this.player.isVisible() && !bomb.isDestroyed()) {
                ...
                if (bombX >= (playerX) && bombX <= (playerX + Commons.PLAYER_WIDTH) && bombY >= (playerY) && bombY <= (playerY + Commons.PLAYER_HEIGHT)) {
                    ...
                    this.player.setDying(true);/*Esta línea ha sido modificada, valor false por true*/
                    bomb.setDestroyed(true); /*Esta línea ha sido modificada, valor false por true*/
                }
            }
            if (!bomb.isDestroyed()) {
                ...
                if (bomb.getY() >= Commons.GROUND - Commons.BOMB_HEIGHT) {
                    ...
                    bomb.setDestroyed(true); //Esta linea ha sido modificada, valor false por true
                }
            }
        }
    }
